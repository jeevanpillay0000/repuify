...
const request = function(method: Method, url: string, body: object | null, timeout: number): Promise<SimpleResponse> {
    ...
    return new Promise((resolve, reject) => {
            ...
            if (xhr.readyState === 4) {
                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        return reject(new Error(`[repu-provider]Error parsing the response: ${e.message}`))
                    }
        ...
        xhr.ontimeout = () => {
            return reject(new Error(`[repu-provider]Time out for whatever reason, check your provider`))
        }

        try {
            xhr.send(method === Method.POST ? JSON.stringify(body) : null)
        } catch (e) {
            return reject(new Error(`[repu-provider]Connect error: ${e.message}`))
        }
    })
}
